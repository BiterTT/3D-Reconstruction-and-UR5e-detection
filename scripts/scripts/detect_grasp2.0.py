...
class UR5GraspSystem:
    ...
    def depth_callback(self, msg):
        try:
            #print("[DEBUG] depth_callback triggered, encoding:", msg.encoding)
            self.depth_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding="passthrough")
            #print("[DEBUG] depth image shape:", self.depth_image.shape)
        except Exception as e:
            #rospy.logerr("Depth image conversion failed: %s", str(e))
            self.depth_image = None

    def image_callback(self, msg):
        if self.processing or self.depth_image is None or self.fx is None:
            return
        ...
        for cnt in contours:
            ...
            valid_window = depth_window[(depth_window > 0) & (depth_window < 1500)]
    ...
    def control_gripper(self, open_gripper=True):
        ...
        point.positions = [0.0 if open_gripper else 0.221]
